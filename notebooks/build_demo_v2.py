...
CELLS.append(code(
    ...
    "    #",
    "    "# Then we tried `get_ipython().run_line_magic('pip', 'install -q ...')`",
    "    "# but that ALSO silently failed because the version spec 'numpy>=1.26,<2.0'",
    "    "# contains a comma that bash splits on — `run_line_magic` returns success but",
    "    "# nothing is installed. Verified in run-1787150113.json.",
    "    #",
    "    "# The fix: use LITERAL %pip lines (not run_line_magic). IPython parses them",
    "    "# natively; the comma-bearing spec is single-quoted so bash doesn't split it.",
    "",
    "    print('Pass 1/2: installing critical deps (ultralytics, supervision, trackers, ...)...')",
    "    # Literal %pip line — IPython handles the magic natively, comma is safe in quotes.",
    "    %pip install -q ultralytics==8.4.121 opencv-python==4.11.0.86 supervision==0.30.0 'numpy>=1.26,<2.0' trackers>=2.6.0",
    "    print('  ✓ Critical deps installed via %pip')",
    "",
    "    # Pass 2: optional — these have NO commas in version specs so we can use",
    "    "# the programmatic form safely. If it fails, the demo still works.",
    "    print()",
