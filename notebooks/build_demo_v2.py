...
CELLS.append(code(
    ...
    "    # The previous single-pass approach had a subtle bug: if one package",
    "    ",
    "    # failed, pip returned non-zero but the cell continued. The user",
    "    # saw 'install succeeded' but ultralytics was actually missing.",
    "    # Now: PASS 1 installs the CRITICAL deps (the demo needs these).",
    "    ",
    "    # If any critical package fails, we RAISE — no silent failures.",
    "    ",
    "    # PASS 2 installs OPTIONAL deps. Failure is OK, we print a warning.",
    "    # The split guarantees that ultralytics/supervision/trackers are",
    "    ",
    "    # always present before the demo continues.",
    "    ",
    "    CRITICAL_PKGS = [",
    "        'ultralytics==8.4.121',",
    ...
    "    ",
    "    # Pass 2: optional — print warning if any fails, but continue",
    "    print()",
    "    print('Pass 2/2: installing optional deps (roboflow, gemini, ...)...')",
    "    opt_result = subprocess.run(",
    "        [sys.executable, '-m', 'pip', 'install'] + OPTIONAL_PKGS,",
    "        capture_output=True, text=True,",
    "    )",
    "    if opt_result.returncode != 0:",
    "        print('⚠ Optional pip install failed (non-fatal). Stderr:')",
    "        print('  ' + (opt_result.stderr or '(empty)').strip()[-300:].replace(chr(10), chr(10) + '  '))",
    "        print('  The demo can still run without these.')",
    "    else:",
    "        print('  ✓ Optional deps installed')",
    "    ",
    ...
    "    if not _import_ok:",
    "        print()",
    "        print('  Critical import failed after install. Re-run the cell.')",
    "        raise SystemExit(0)",
    "    print()",
