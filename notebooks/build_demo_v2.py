...
CELLS.append(code(
    ...
    "    # Note: `trackers` is the Roboflow ByteTrack package (Apache 2.0) —",
    "    # replaces the deprecated supervision.ByteTrack. Without it, the",
    "    # pipeline falls back to the simple IoU tracker (works, but worse IDs).",
    "    # We use capture_output and check returncode (not check=True) so a",
    "    # single package failure (e.g. a Colab pre-installed dep conflict)",
    "    # doesn't kill the cell — we print the stderr for debugging and",
    "    # continue with whatever installed successfully.",
    ...
    "        'trackers>=2.6.0',  # ByteTrack multi-object tracker (Roboflow, Apache 2.0)",
    "    ], capture_output=True, text=True)",
    "    if install_result.returncode != 0:",
    "        print('⚠ pip install returned non-zero exit code 1. Stderr (last 500 chars):')",
    "        stderr = (install_result.stderr or '(empty)').strip()",
    "        for line in stderr[-500:].split(chr(10))[-15:]:",
    "            print(f'  {line}')",
    "        print()",
    "        print('  Common causes on Colab:')",
    "        print('  - Pre-installed package conflicts with our pins (e.g. numpy 2.x vs our <2.0)')",
    "        print('  - Network timeout (retry in a minute)')",
    "        print('  - A package version that no longer exists on PyPI')",
    "        print()",
    "        print('  Continuing with whatever installed successfully. The pipeline can still run.')",
    "    else:",
    "        print('✓ Pinned deps installed (including trackers for ByteTrack)')",
    "",
