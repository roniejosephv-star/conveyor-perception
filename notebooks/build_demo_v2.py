...
CELLS.append(code(
    "# --- Cell 7.6: Download 2nd dataset (Roboflow, with fallback) ---",
    "import os, sys",
    "from pathlib import Path",
    "from colab_session import get_state",
    "",
    ...
    "",
    "# 1. If already on disk, skip.",
    "if DATA_YAML.exists():",
    "    n = 0",
    "    td = TARGET / 'train' / 'images'",
    "    if td.exists():",
    "        n = len(list(td.glob('*.jpg'))) + len(list(td.glob('*.png')))",
    "    print(f'  recycling_v3 already on disk ({n} train images) — skipping download.')",
    "    state.metric('dataset_recycling_v3_status', 'already_present')",
    "    state.log('cell-7.6', action='skip', reason='already-present')",
    "else:",
    "    print('  recycling_v3 not found on disk. Trying Roboflow Universe...')",
    "    state.metric('dataset_recycling_v3_status', 'downloading')",
    "    _ok = False",
    "    _err = None",
    "    try:",
    ...
    "        _project = _rf.workspace('zkf624').project('-recycling')",
    "        _version = _project.version(3)",
    ...
    "        if td.exists():",
    "            n = len(list(td.glob('*.jpg'))) + len(list(td.glob('*.png')))",
    "        print(f'  Downloaded recycling_v3 ({n} train images, CC BY 4.0)')",
    "        state.metric('dataset_recycling_v3_status', 'downloaded')",
    "    else:",
    "        print(f'  Roboflow download failed: {_err}')",
    "        print()",
    "        print('  Manual fallback (one-time setup):')",
    "        print('    1. Open https://universe.roboflow.com/zkf624/-recycling/dataset/3')",
    "        print('    2. Click Download Dataset -> Format: YOLOv11 -> Show Download Code')",
    "        print(f'    3. Extract the zip into {DATA_RAW}/ so the path is:')",
    "        print(f'       {DATA_RAW}/recycling_v3/data.yaml  (must contain data.yaml)')",
    "        print()",
    "        print('  The demo continues with recycling_demo (bundled) only until you do this.')",
    "        state.metric('dataset_recycling_v3_status', 'failed')",
    "        state.log('cell-7.6', action='download-failed', error=str(_err))",
    "",
    "# 2. Re-run the registry to pick up the new dataset (if any).",
    ...
    "        _nt = _cnt(_cfg.get('train', 'train/images'))",
    "        _nv = _cnt(_cfg.get('val',   'val/images'))",
    ...
    "        _registry.append({'name': _d.name, 'status': _st, 'n_train': _nt, 'n_val': _nv,",
    "                          'nc': _cfg.get('nc', len(_names)), 'names': _names,",
    "                          'source': _m.get('source', '-'), 'license': _m.get('license', '-'),",
    "                          'baseline_mAP50': _m.get('pre_trained_baseline_mAP50')})",
    "state.dataset_registry = _registry",
    "",
    "print()",
    "print('-' * 72)",
    "print(f'  DATASETS NOW AVAILABLE: {len(_registry)}'.center(72))",
    "print('-' * 72)",
    "for _r in _registry:",
    "    print(f'  - {_r[\"name\"]:20}  {_r[\"status\"]:11}  {_r[\"n_train\"]:>5} train / {_r[\"n_val\"]:>4} val  ({_r[\"nc\"]} cls)')",
    "print('-' * 72)",
    "print('  Next: cell 8 (train on a dataset) - re-run to cache, no retrain.')",
))
