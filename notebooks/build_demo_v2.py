...
CELLS.append(code(
    ...
    "        # Read data.yaml — YOLO needs this to find train/val/test splits",
    "        yaml_candidates = list(DATA_RAW.rglob('data.yaml'))",
    "        if not yaml_candidates.exists() if isinstance(yaml_candidates, bool) else not yaml_candidates:",
    "            print(f'✗ No data.yaml found in {DATA_RAW}')",
    "            print('  Run cell 2 first to copy the bundled data.')",
    "            raise SystemExit(0)",
    "        yaml_path = yaml_candidates[0]",
    "        data_cfg = yaml.safe_load(yaml_path.read_text())",
    "        print(f'  Dataset: {len(list((DATA_RAW / \"train\" / \"images\").glob(\"*.jpg\"))) + len(list((DATA_RAW / \"train\" / \"images\").glob(\"*.png\")))} train images, 4 classes ({data_cfg.get(\"names\")})')",
    "",
