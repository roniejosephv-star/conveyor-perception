...
CELLS.append(code(
    ...
    "",
    "# --- 5. Make sure the repo is on the Python path so the next cells can `import` ---",
    ...
    "        REPO = REPO.parent",
    "# Add BOTH REPO and REPO/notebooks — colab_session.py lives in notebooks/,",
    "# not at the repo root. Without this, the `import colab_session` below fails",
    "# with ModuleNotFoundError on Colab (caught + fixed Aug 22 2026).",
    "for _p in (REPO, REPO / 'notebooks'):",
    "    _sp = str(_p)",
    "    if _sp not in sys.path:",
    "        sys.path.insert(0, _sp)",
    "print(f'  Repo:   {REPO}')",
    "print(f'  Path:   {REPO}/notebooks  (colab_session.py lives here)')",
    "",
    "# --- 6. State singleton (re-used by every cell that needs to log/metric) ---",
    "from colab_session import get_state",
    "state = get_state()",
    "state.metric('python', sys.version.split()[0])",
    "state.metric('gpu', _gpu)",
    "state.metric('in_colab', int(IN_COLAB))",
    "print()",
    "print('  ✓ env check done.  Next: cell 2 (install + clone if needed).')",
))
