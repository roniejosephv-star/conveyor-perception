...
# ---------------------------------------------------------------------------
# Cell 12 (code): Production path — Roboflow Inference (library mode).
# Same model family, different runtime. This is what runs on the edge in prod
# (HTTP server, Workflows DSL, Jetson). Self-healing: if `inference` isn't
# installed (its deps conflict with our pinned versions), the cell prints a
# skip + install hint and continues.
# ---------------------------------------------------------------------------
CELLS.append(code(
    ...
    "",
    "        # --- 2. Load via Roboflow Inference (foundation model OR your uploaded model) ---",
    "        # Priority: ROBOFLOW_MODEL_ID from env var (or Colab userdata) → 'yolov8n-640'.",
    "        # 'yolov8n-640' is the COCO YOLOv8n foundation alias. It's a DIFFERENT model",
    "        # from our recycling_v3 (YOLO26s, 4-class) — the point of this cell is the",
    "        # *runtime* difference (Ultralytics vs Roboflow Inference), not model parity.",
    "        # For apples-to-apples: one-time setup, then demo runs automatically:",
    "        #   1. Upload best.pt to https://app.roboflow.com/ (manual, 5 min)",
    "        #   2. Copy the workspace/project/version ID (e.g. 'your-ws/recycling-v3/1')",
    "        #   3. Set it once:",
    "        #        Colab:   from google.colab import userdata; userdata.set('ROBOFLOW_MODEL_ID', 'your-ws/recycling-v3/1')",
    "        #        Local:   export ROBOFLOW_MODEL_ID='your-ws/recycling-v3/1'",
    "        #   After that, every demo run uses your uploaded weights automatically.",
    "        _model_id = 'yolov8n-640'  # placeholder fallback",
    "        try:",
    "            if IN_COLAB:",
    ...
    "            print(f'  Loading your uploaded model: {_model_id}  (via Roboflow Inference)')",
    "        else:",
    "            print(f'  Loading foundation model: {_model_id}  (COCO YOLOv8n placeholder)')",
    "            print('  For your uploaded recycling_v3: set ROBOFLOW_MODEL_ID per the comment above.')",
